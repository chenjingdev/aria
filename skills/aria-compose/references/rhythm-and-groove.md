...
groove는 “노트를 조금씩 틀리게 놓는 것”이 아니다. 명확한 pulse, 예상 가능한 몸의 기준, 적절한 싱코페이션, 파트 사이의 관계, 음색의 어택(Attack, 소리가 시작되는 성질)이 함께 만든다. [S18][S60]
...
- velocity 차이를 크게 쓸 때 프리셋과 `velRange`가 샘플 엔진에 전달하는 강약 레이어·음색 반응을 듣는다. `velRange:0`은 모든 노트를 MIDI velocity 64로, `1`은 악보 값을 그대로 보낸다.
