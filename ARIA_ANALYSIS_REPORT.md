...
> **역사적 스냅샷 — 2026-08-08 이전 구현 기준.** 아래 본문과 줄 번호는 당시의 내장 합성 신스와 `src/synth.js`를 분석한 기록이라 현재 코드 설명으로 사용하면 안 된다. 2026-08-08부터 Aria는 내장 선율 신스 13개와 합성 드럼 3개를 제거했고, 외부 SoundFont만 `src/renderer.js`와 `src/sf2.js`로 재생한다. 현재 동작은 README와 실제 소스·테스트를 우선한다.
