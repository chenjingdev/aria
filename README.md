...
**v0.1.0-alpha.2 · macOS용 AI 작곡 프리뷰**
...
4. **시작 안내 → AI에게 연결 맡기기 → 연결 요청 복사**를 눌러 이 Mac에서 사용하는 Codex·Claude Code 대화에 붙여넣으세요. 에이전트가 현재 설치 경로로 MCP를 등록하고 실제 Aria 도구 호출을 확인합니다. 해당 AI 도구 설치·로그인은 먼저 완료되어 있어야 합니다. [Claude Code 시작 안내](https://code.claude.com/docs/en/quickstart)
5. 에이전트가 새 대화가 필요하다고 안내하면 새 대화에서 **확인 요청**을 보내세요. Aria에 실제 도구 요청이 도착하면 최근 요청 시각이 표시됩니다. 이어서 **첫 곡 요청**을 복사해 보내면 됩니다. 직접 설정할 때의 명령과 JSON은 접힌 **직접 연결하기**에 있습니다.

음원이 이미 있어도 **안내 마치기**를 누르기 전까지는 시작 안내가 자동으로 열립니다. ×로 닫으면 이번 창만 닫고, **안내 마치기**를 누르면 다음 실행부터 자동으로 열리지 않습니다. 상단 **시작 안내**로 언제든 다시 볼 수 있습니다.
